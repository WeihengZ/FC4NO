...
def create_data_loaders(graph_folder_path, batch_size=1):
    ...
    # # compute Max and Min in three direction
    # max_over_all_graphs = None
    # min_over_all_graphs = None
    # for k in range(num_samples):
    #     if k % 10 == 0:
    #         print(k)
    #     graph = torch.load(graph_paths[k], weights_only=False)
    #     coor = graph.x  # (num_nodes, 3) — assuming coordinates are stored in graph.x
    #     max_vals = torch.amax(coor, dim=0)
    #     min_vals = torch.amin(coor, dim=0)
    #     if max_over_all_graphs is None:
    #         max_over_all_graphs = max_vals
    #         min_over_all_graphs = min_vals
    #     else:
    #         max_over_all_graphs = torch.maximum(max_over_all_graphs, max_vals)
    #         min_over_all_graphs = torch.minimum(min_over_all_graphs, min_vals)
    # bound_info = (max_over_all_graphs.numpy().tolist(), min_over_all_graphs.numpy().tolist())
