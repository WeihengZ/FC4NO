...
data_loc = '/taiga/illinois/eng/cee/meidani/Vincent/bracket_static/'
# Specify the output graph location
save_path = '/taiga/illinois/eng/cee/meidani/Vincent/bracket_static/Processed/'

# read the raw data
params = np.load(os.path.join(data_loc, 'input_params.npy'))    # (B, 4)
with open(os.path.join(data_loc, 'LUG_node_S_PC.pkl'), "rb") as f:
    data = pickle.load(f)
...
def min_max_scale(params, vertices, nodal_stress, points_bc):
    ...
    np.savez(os.path.join(save_path, "min_max.npz"), min=min_stress_features, max=max_stress_features)
...
# create graphs
graph_rep_generate(params, vertices, nodal_stress, points_bc, save_path)