...
def detect_edge_segments(points, k_neighbors=8, curvature_threshold=0.07, edge_link_neighbors=3):
    """
    Detect edges from a point cloud and return edge segments.

    Args:
        points (np.ndarray): Input point cloud of shape (N, 3)
        k_neighbors (int): Number of neighbors for PCA analysis
        curvature_threshold (float): Curvature threshold to determine edge points
        edge_link_neighbors (int): Number of neighbors to connect for each edge point

    Returns:
        np.ndarray: Edge list of shape (E, 6) where each row is [x1, y1, z1, x2, y2, z2]
    """
    N = points.shape[0]
    tree = KDTree(points)
    ...
    for i in edge_indices:
        _, neighbors = tree.query(points[i], k=edge_link_neighbors + 1)
        for j in range(1, edge_link_neighbors + 1):
            a = points[i]
            b = points[neighbors[j]]
            edge_segments.append(np.concatenate([a, b]))

    return np.array(edge_segments)
