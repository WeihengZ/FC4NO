...
# read the raw data
data_loc = '/work/hdd/bdsy/wzhong/GEJetEngineBracket/'
with open(os.path.join(data_loc, 'GE-JEB.pkl'), "rb") as f:
    data = pickle.load(f)
vertices = data['vertices']    # list of (M,3), len=B
nodal_stress = data['nodal_stress']    # list of (M), len=B
points_bc = data['points_cloud']    # list of (N,3), len=B
...
def min_max_scale(vertices, nodal_stress, points_bc):
    ...
    mean_stress_features = np.mean(all_stress)
    std_stress_features = np.std(all_stress)
    print('mean_stress_features:', mean_stress_features)
    print('std_stress_features:', std_stress_features)
    ...
    for i in range(len(vertices)):
        ...
        nodal_stress[i] = (nodal_stress[i] - mean_stress_features) / std_stress_features
...
# graph process
def graph_rep_generate(points_list, nodal_stress, points_bc, save_path, debug=True):
    '''
...
save_file = '/work/hdd/bdsy/wzhong/processed_data/jet2/'
graph_rep_generate(vertices, nodal_stress, points_bc, save_file, debug=True)