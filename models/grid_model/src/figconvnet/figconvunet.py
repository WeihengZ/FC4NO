...
class FIGConvUNet(BaseModel):
    ...
    def _grid_forward(self, point_features: PointFeatures):
        ...
        # grid_feature_group.grid_features is a list of GridFeatures (length=3)
        ...
        # down_grid_feature_groups is a list of grid_feature_group, length = 3
        # length of grid_feature_group[i] = 3
        # grid_feature_group[i][j].vertices/features is tensor
        # features are being downsized here (last two dimension is half): [0][j] -> [1][j] -> [2][j]
        down_grid_feature_groups = [grid_feature_group]
        for down_block in self.down_blocks:
            # pick the last grid_feature_group as input to down size
        ...
        '''
        down_grid_feature_groups[-1] shape:
            torch.Size([1, 80, 38, 25])
            torch.Size([1, 48, 63, 25])
            torch.Size([1, 32, 63, 38])
        '''
        # upsampling
        # level as an index is counted reversely
        for level in reversed(range(self.num_levels)):

            # for j in range(3):
            #     print(down_grid_feature_groups[level + 1][j].vertices.shape)
            #     print(down_grid_feature_groups[level + 1][j].features.shape)
            # assert 1==2
        ...
        # grid_features is a list of GridFeatures (length=3)
        '''
        original shape:
        (torch.Size([1, 5, 150, 100, 3]), torch.Size([1, 80, 150, 100]))
        torch.Size([1, 250, 3, 100, 3]), torch.Size([1, 48, 250, 100])
        ...
        '''
        current shape
        torch.Size([1, 5, 150, 100, 3]) torch.Size([1, 5, 152, 100, 16])
        torch.Size([1, 250, 3, 100, 3]) torch.Size([1, 252, 3, 100, 16])
        torch.Size([1, 250, 150, 2, 3]) torch.Size([1, 252, 152, 2, 16])
        '''
