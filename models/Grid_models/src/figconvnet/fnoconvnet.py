...
class FIGConvUNet(BaseModel):
    ...
    def __init__(
        ...
        self.fno = FNO3d(modes1=fno_mode, modes2=fno_mode, modes3=fno_mode, width=self.dim)
        self.mllp = nn.Linear(self.dim, 1)
        # only use for features
        self.feature_encoder = nn.Sequential(
            nn.Linear(in_channels, 3*self.dim),
            nn.GELU(),
    ...
    def _grid_forward(self, point_features: PointFeatures):
        ...
        '''  
        # B_, PP_, M_, N_ = down_grid_feature_groups[0][0].features.shape
        # G = down_grid_feature_groups[0][0].features.reshape(B_,int(PP_/self.dim),self.dim,M_,N_)
        # G = G.permute(0,1,3,4,2)
        G = down_grid_feature_groups[0][0].features

        # print(G.shape)
        # assert 1==2

        G = self.fno(G)
        # G = G.permute(0,1,4,2,3)
        # G = G.reshape(B_, PP_, M_, N_)
        down_grid_feature_groups[0][0].features = G
