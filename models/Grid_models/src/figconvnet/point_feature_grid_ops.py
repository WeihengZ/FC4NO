...
class PointFeatureToGrid(nn.Module):
    ...
    def forward(self, point_features: PointFeatures) -> GridFeatures:
        ...
        # out shape PointFeatures(vertices = (1,M,3), features = (1,M,F_origin))
        # M = gridx * gridy * gridz
        grid_point_features = self.grid_features.point_features.expand_batch_size(
            point_features.batch_size
        )

        # out shape PointFeatures(vertices = (1,M,3), features = (1,M,F))
        out_point_features = self.conv(
            point_features,
            grid_point_features,
        )

        # shape of grid_feature:
        #   vertices=(1,M,N,K,3), features=(1,M,N,K,F)
...
class GridFeatureToPoint(nn.Module):
    ...
    def __init__(
        ...
        sample_method: Literal["graphconv", "interp", "attn"] = "graphconv",
        neighbor_search_type: Literal["radius", "knn"] = "radius",
        knn_k: int = 16,
        reductions: List[REDUCTION_TYPES] = ["mean"],
    ) -> None:
        ...
        elif sample_method == "attn":
            self.conv = GridFeatureToPointAttn(
                grid_in_channels,
                point_in_channels,
                out_channels,
                aabb_max,
...
Added
'''
class GridFeatureToPointAttn(nn.Module):
    """GridFeatureToPointAttn."""
    ...
    def attn_forward(self,
        ...
        print(neighbor_search_vertices_scaler)
        assert 1==2
    ...
    def forward(
        ...
        out_point_features = self.attn_forward(
